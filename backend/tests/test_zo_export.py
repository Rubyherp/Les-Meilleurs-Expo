...
async def test_documented_ask_endpoint_confirms_save():
    ...
    file_path = (
        ...
        f"{value.session_id}-{artifact.idempotency_key[:16]}.json"
    )
    expected_sha256 = hashlib.sha256(report_json.encode()).hexdigest()
    ...
    async def handler(request):
        ...
        if len(calls) == 1:
            output = {
                "written": True, "file_path": file_path, "url": "", "message": "Saved"
            }
        else:
            assert "conversation_id" not in payload
            output = {"exists": True, "file_path": file_path, "sha256": expected_sha256}
        return httpx.Response(200, json={"output": output, "conversation_id": "conv-1"})
    http = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    response = await ZoClient(Settings(zo_api_key="test"), http).export_to_zo(value.session_id, artifact)
    await http.aclose()
    assert response.status == "completed"
    assert response.export_id == artifact.idempotency_key[:16]
...
async def test_full_ask_endpoint_is_not_appended_twice():
    ...
    async def handler(request):
        ...
        output = (
            {"written": True, "file_path": file_path, "url": "", "message": "Saved"}
            if count == 1
            else {"exists": True, "file_path": file_path, "sha256": expected_sha256}
        )
        return httpx.Response(200, json={"output": output, "conversation_id": "conv-1"})
    http = httpx.AsyncClient(transport=httpx.MockTransport(handler))
...
@pytest.mark.asyncio
async def test_hash_mismatch_does_not_claim_report_was_saved():
    count = 0
    value = report()
    artifact = build_zo_artifact(value.session_id, value)
    file_path = (
    ...
    async def handler(request):
        ...
        output = (
            {"written": True, "file_path": file_path, "url": "", "message": "Saved"}
            if count == 1
            else {"exists": True, "file_path": file_path, "sha256": "wrong"}
        )
        return httpx.Response(200, json={"output": output, "conversation_id": "conv-1"})

    http = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    response = await ZoClient(Settings(zo_api_key="test"), http).export_to_zo(
        value.session_id, artifact
    )
    await http.aclose()
    assert response.status == "failed"
    assert response.file_path is None
    assert "readback verification failed" in response.message
...
async def test_reminder_is_only_created_when_explicit():
    ...
    async def handler(request):
        ...
        count += 1
        if count == 1:
            output = {"written": True, "file_path": file_path, "url": "", "message": "Saved"}
        elif count == 2:
            output = {"exists": True, "file_path": file_path, "sha256": expected_sha256}
        else:
